...
{ 
  ...
  HAL_DMA_Init(&UART2TxDMA_Handler);
	...
	//Rx DMA配置
	UART2RxDMA_Handler.Instance=DMA1_Stream5;                            //数据流选择
	UART2RxDMA_Handler.Init.Channel=DMA_CHANNEL_4;                                //通道选择
...
void DMA_Init(void)
{
	...
	//UART2 DMA config
	UART2_DMA_Config();
	HAL_NVIC_SetPriority(DMA1_Stream6_IRQn, 1, 4);       //抢占优先级为1，子优先级为4
	HAL_NVIC_EnableIRQ(DMA1_Stream6_IRQn);
	HAL_NVIC_SetPriority(DMA1_Stream5_IRQn, 1, 3);       //抢占优先级为1，子优先级为5	
	HAL_NVIC_EnableIRQ(DMA1_Stream5_IRQn);
	//USART2启动DMA接收
	HAL_UART_DMAStop(&UART2_Handler);//先关一下，否则DMA接收不到第一包数据（不管几字节）
